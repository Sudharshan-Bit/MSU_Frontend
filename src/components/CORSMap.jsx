...
const CORSMap = ({ onLocationFound, outputData, coordinates }) => {
  ...
  const[symbolType,setSymbolType]=useState("object");
  ...
  useEffect(() => {
    ...
    // Common styles and reusable functions
    const commonOutline = { color: "white", width: 1 };

    // Function to create 3D symbols dynamically
    const getObjectSymbol = (color = "blue", primitive = "cylinder", width = 10000, height = 100000) => ({
      type: "point-3d",
      symbolLayers: [
      {
        type: "object",
    ...
    // Function to create simple marker symbols dynamically
    const createSimpleMarkerSymbol = (color, size = 8) => ({
    type: "simple-marker",
    color,
    size,
    outline: commonOutline,
    });
   // Renderer for general STATUS with is3D condition
   ...
   const renderer = {
    ...
    uniqueValueInfos: [
      {
        value: "Present",
        symbol: is3D && symbolType === "object"
          ? getObjectSymbol("blue", "cone", 8000, 80000) // 3D blue cylinder
          : createSimpleMarkerSymbol("blue"), // 2D blue marker
        label: `Present (${presentCount})`,
      },
      {
        value: "Not Present",
        symbol: is3D && symbolType === "object"
          ? getObjectSymbol("red", "cone", 8000, 80000) // 3D red cylinder
          : createSimpleMarkerSymbol("red"), // 2D red marker
   ...
   // Renderer for uncertainty or MYCS2 Prediction
   const renderer_1 = {
    type: "unique-value",
    field: "STATUS",
    uniqueValueInfos: [
      {
        ...
        symbol: is3D && symbolType === "object"
        ? getObjectSymbol("yellow", "cylinder", 8000, 8000)
        : createSimpleMarkerSymbol("yellow"), // 2D orange marker
        label: uncertainty_status
          ? `Uncertainty (${presentCount})`
          : `MYCS2 Prediction (${presentCount})`,
        },
        {
          value: "Observation",
          symbol: is3D && symbolType === "object"
          ? getObjectSymbol("green","cylinder",8000,8000) 
          : createSimpleMarkerSymbol("green"), // 2D green marker
          label: `Observation (${notPresentCount})`,
        },
      ],
    };

    // Create the first GeoJSON layer (for Present and Not Present statuses)
    const geojsonLayer = new GeoJSONLayer({
      url: url,
      popupTemplate: template,
      // Use renderer_1 for handling uncertainty and MYCS2 prediction
      renderer: prediction_status || uncertainty_status ? renderer_1 : renderer,
      orderBy: {
        field: "STATUS"
      }
    });
  ...
  }, [onLocationFound, outputData, fetchedData, loading,bg_loader,is3D,symbolType]);  // Initialize map only once

  const handleSymbolChange = (event) => {
    setSymbolType(event.target.value)
    console.log(symbolType)
    // Call function to update the map renderer based on symbolType
  };

  // const updateRenderer = () => {
  //   // Logic to update the map renderer dynamically based on `symbolType` and `is3D`
  //   // E.g., Use GeoJSONLayer and SceneView/MapView based on the state
  //   console.log(`Renderer updated: ${symbolType} in ${is3D ? "3D" : "2D"} view`);
    
  // };

  const switchView = () => {
    setIs3D(!is3D);
    // Logic to toggle between SceneView and MapView
...
//     mapViewRef.current = view;
  ...
  return (
    <div>
      ...
      <div ref={mapRef} className="h-[88vh] w-full">
      <div
          ref={miniMapRef}
          className="absolute  bottom-40 right-4 h-40 w-40 border-2 border-gray-700 z-10"
        ></div>
        </div>  {/* Attach the map view to this div */}
      ...
      <button
        ...
        >
         ...
         {/* Control Panel */}
          {is3D && ( // Only render when `is3DView` is true
          <div
          style={{
          position: "absolute",
          top: 400,
